...
namespace Commons
{
    ...
    public class ImageProperties
    {
        ...
        public Int32 ColorDepth = 0;
        public Int32 NumColorsInPalette = 0;
    }
    ...
    public static class ImageUtils
    {
        ...
        */
        ...
        public static ImageProperties GetImageProperties(Byte[] imageData, ImageFormat format = ImageFormat.Undefined)
        {
            ...
            {
                var limit = (Int64)Math.Round(s.Length * 0.25); // TODO - test and adjust limit

                switch (format)
                {
                    case (ImageFormat.Tiff):
                        var isBigEndian = (0x4D == r.ReadByte());
                        s.Seek(3, SeekOrigin.Current); // Skip the rest of the signature
                        Int64 IFDOffset = readInt32(r, isBigEndian);

                        s.Seek(IFDOffset, SeekOrigin.Begin);

                        Int32 nbIFDEntries = readInt16(r, isBigEndian);

                        var initialPos = s.Position;
                        Int32 IFDtag, IFDFieldType, IFDNbValues, IFDValue32, IFDValue16;
                        Byte[] IFDValueBinary;
                        var photometricInterpretation = 0;
                        var bitsPerSample = 0;
                        var samplesPerPixel = 0;

                        for (var i=0; i<nbIFDEntries; i++)
                        {
                    ...
                    case (ImageFormat.Gif):
                        ...
                        if (0x2c == r.ReadByte())
                        {
                            ...
                            {
                                props.NumColorsInPalette = (Int32)Math.Pow(2, ((globalPaletteUse & 0x0F) << 4) + 1);
                            }
                        } else
                        {
                    ...
                    case (ImageFormat.Bmp):
                        ...
                        // No support for BMP color palettes, as they seem to be exotic (and ModifiedAtl has no use of this information)

                        break;

                    case (ImageFormat.Png):
                        var intData = new Byte[4];
                        var IHDRChunkSignature = Utils.Latin1Encoding.GetBytes("IHDR");
                        var PaletteChunkSignature = Utils.Latin1Encoding.GetBytes("PLTE");
                        ...
                        else
                        {
                            ...
                            if (3 == colorType) // PNG file uses a palette
                            {
                                ...
                                {
                                    props.NumColorsInPalette = (Int32)Math.Floor(paletteChunkSize / 3.0);
                                }
                            }
                            else
                            {
                    ...
                    case (ImageFormat.Jpeg):
                        ...
                        var SOF0FrameSignature = new Byte[2] { 0xFF, 0xC0 };
        ...
        private static Int16 readInt16(BinaryReader r, Boolean isBigEndian)
        {
