...
namespace ATL
{
    ...
    {
        private const Int32 DEFAULT_BUFFER_SIZE = 512;

        private readonly Stream stream;
        private readonly Int32 bufferDefaultSize;
        private readonly Int64 streamSize;
         ...
         */
        private Byte[] buffer;
        private Int64 bufferOffset;
        private Int32 cursorPosition; // NB : cursorPosition can be > bufferSize in certain cases when BufferedBinaryReader has to read a chunk of data larger than bufferSize
        private Int64 streamPosition;
        private Int32 bufferSize; // NB : bufferSize can be < DEFAULT_BUFFER_SIZE when bufferOffset nears the end of the stream (not enough remaining bytes to fill the whole buffer space)
        ...
        public override Int64 Length => streamSize;
        ...
        public BufferedBinaryReader(Stream stream, Int32 bufferSize)
        {
            this.stream = stream;
        ...
        private Boolean fillBuffer(Int32 previousBytesToKeep = 0)
        {
            if (previousBytesToKeep > 0) Array.Copy(buffer, cursorPosition, buffer, 0, previousBytesToKeep);
            var bytesToRead = (Int32)Math.Min(bufferDefaultSize-previousBytesToKeep, streamSize - streamPosition - previousBytesToKeep);
        ...
        private void prepareBuffer(Int32 bytesToRead)
        {
        ...
        {
            Int64 delta = 0; // Distance between absolute cursor position and specified position in bytes
        ...
        public override Int32 Read([In, Out] Byte[] buffer, Int32 offset, Int32 count)
        {
            ...
            else
            {
                ...
                var availableBytes = bufferSize - cursorPosition;
                if (availableBytes > 0)
                {
        ...
        public Byte[] ReadBytes(Int32 nbBytes)
        {
        ...
        {
            prepareBuffer(2);
            var val = (UInt16)(buffer[cursorPosition] | buffer[cursorPosition + 1] << 8);
            cursorPosition += 2;
            return val;
        }
        ...
        {
            prepareBuffer(2);
            var val = (Int16)(buffer[cursorPosition] | buffer[cursorPosition + 1] << 8);
            cursorPosition += 2;
            return val;
        }
        ...
        {
            prepareBuffer(4);
            var val = (UInt32)(buffer[cursorPosition] | buffer[cursorPosition + 1] << 8 | buffer[cursorPosition + 2] << 16 | buffer[cursorPosition + 3] << 24);
            cursorPosition += 4;
            return val;
        }
        ...
        {
            prepareBuffer(4);
            var val = (buffer[cursorPosition] | buffer[cursorPosition + 1] << 8 | buffer[cursorPosition + 2] << 16 | buffer[cursorPosition + 3] << 24);
            cursorPosition += 4;
            return val;
        }
        ...
        {
            prepareBuffer(8);
            var val = (UInt64)(buffer[cursorPosition] | buffer[cursorPosition + 1] << 8 | buffer[cursorPosition + 2] << 16 | buffer[cursorPosition + 3] << 24 | buffer[cursorPosition + 4] << 32 | buffer[cursorPosition + 5] << 40 | buffer[cursorPosition + 6] << 48 | buffer[cursorPosition + 7] << 56);
            cursorPosition += 8;
            return val;
        }
        ...
        {
            prepareBuffer(8);
            var val = (Int64)(buffer[cursorPosition] | buffer[cursorPosition + 1] << 8 | buffer[cursorPosition + 2] << 16 | buffer[cursorPosition + 3] << 24 | buffer[cursorPosition + 4] << 32 | buffer[cursorPosition + 5] << 40 | buffer[cursorPosition + 6] << 48 | buffer[cursorPosition + 7] << 56);
            cursorPosition += 8;
            return val;
        }
