...
namespace ATL
{
	...
	public class TagData
	{
        ...
        public delegate void PictureStreamHandlerDelegate(ref MemoryStream stream, PictureInfo.PIC_TYPE picType, ImageFormat imgFormat, Int32 originalTag, Object nativePicCode, Int32 position);
        ...
        {
            var picturePositions = generatePicturePositions();

            // String values
            var newData = data.ToMap();
            ...
            {
                foreach (var newPicInfo in data.Pictures)
                {
                    ...
                    if (picturePositions.ContainsKey(newPicInfo))
                    {
                        ...
                        {
                            foreach (var picInfo in Pictures)
                            {
            ...
            foreach (var newMetaInfo in data.AdditionalFields)
            {
                found = false;
                foreach (var metaInfo in AdditionalFields)
                {
        ...
        private IDictionary<PictureInfo,Int32> generatePicturePositions()
        {
            IDictionary<PictureInfo, Int32> result = new Dictionary<PictureInfo, Int32>();
