...
namespace ATL.AudioData
{
	...
	public class MetaDataIOFactory
	{
        ...
        public const Int32 TAG_ID3V1 = 0;
		public const Int32 TAG_ID3V2 = 1;
		public const Int32 TAG_APE = 2;
        public const Int32 TAG_NATIVE = 3;    // Native tag format associated with the audio container (ex : MP4 built-in tagging format)
        public const Int32 TAG_ANY = 99;      // Whenever tag type is not known in advance and may apply to any available tag

        // Count of the types defined above, excluding "any" type
        public static Int32 TAG_TYPE_COUNT = 4;

        // Defines the default reading priority of the metadata
        private Int32[] tagPriority;

		// Defines whether the next created metadatareaders should use cross-tag reading
		private Boolean m_enableCrossReading = true;
        ...
        /// </summary>
		public Boolean CrossReading
		{
			get => m_enableCrossReading;
			set => m_enableCrossReading = value;
		}

        public Int32[] TagPriority
        {
            get => tagPriority;
		...
		{
            if (!BitConverter.IsLittleEndian) throw new PlatformNotSupportedException("Big-endian based platforms are not supported by ModifiedAtl");

            if (null == theFactory)
			{
				theFactory = new MetaDataIOFactory();
                theFactory.tagPriority = new Int32[TAG_TYPE_COUNT];
		...
		public void SetTagPriority(Int32 tag, Int32 rank)
		{
        ...
        public IMetaDataIO GetMetaReader(AudioDataManager theDataManager, Int32 forceTagType = -1)
		{
			...
			{
				for (var i=0; i<TAG_TYPE_COUNT; i++)
				{
