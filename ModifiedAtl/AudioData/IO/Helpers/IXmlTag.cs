...
namespace ATL.AudioData.IO
{
    public static class IXmlTag
    {
        ...
        public static void FromStream(Stream source, MetaDataIO meta, ReadTagParams readTagParams, UInt32 chunkSize)
        {
            ...
            using (var mem = new MemoryStream((Int32)chunkSize))
            { 
                StreamUtils.CopyStream(source, mem, (Int32)chunkSize); // Isolate XML structure in a clean memory chunk
        ...
        public static Int32 ToStream(BinaryWriter w, Boolean isLittleEndian, MetaDataIO meta)
        {
            ...
            var writer = XmlWriter.Create(w.BaseStream, settings);
            ...
            foreach(var key in additionalFields.Keys)
            {
                if (key.StartsWith("ixml."))
                {
                    ...
                    for (var i = previousPath.Length - 2; i >= 0; i--)
                    {
