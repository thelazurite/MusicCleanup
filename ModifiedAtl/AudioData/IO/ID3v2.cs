...
namespace ATL.AudioData.IO
{
    ...
    ///     
    ///     This feature is currently not supported. If any CTOC is detected while reading, ModifiedAtl will "blindly" write a flat CTOC containing
    ...
    public class ID3v2 : MetaDataIO
    {
        ...
        // Mapping between ID3v2 field IDs and ModifiedAtl fields
        ...
        private class TagInfo
        {
            ...
            public Int64 HeaderEnd;                           // End position of header
            public Int64 ActualEnd;     // End position of entire tag (including all padding bytes)

            // Extended header flags
            public Int32 ExtendedHeaderSize = 0;
            public Int32 ExtendedFlags;
            public Int32 CRC = -1;
            public Int32 TagRestrictions = -1;


            // **** BASE HEADER PROPERTIES ****
            public Boolean UsesUnsynchronisation => ((Flags & 128) > 0);

            public Boolean HasExtendedHeader // Determinated from flags; indicates if tag has an extended header (ID3v2.3+)
                =>
                    (((Flags & 64) > 0) && (Version > TAG_VERSION_2_2));

            public Boolean IsExperimental // Determinated from flags; indicates if tag is experimental (ID3v2.4+)
                =>
                    ((Flags & 32) > 0);

            public Boolean HasFooter // Determinated from flags; indicates if tag has a footer (ID3v2.4+)
                =>
                    ((Flags & 0x10) > 0);

            // **** EXTENDED HEADER PROPERTIES ****
            public Int32 TagFramesRestriction
            {
                get
                {
            ...
            public Int32 TagSizeRestrictionKB
            {
                get {
                    switch ((TagRestrictions & 0xC0) >> 6)
                    {
            ...
            public Boolean HasTextEncodingRestriction => (((TagRestrictions & 0x20) >> 5) > 0);

            public Int32 TextFieldSizeRestriction
            {
                get
                {
            ...
            public Boolean HasPictureEncodingRestriction => (((TagRestrictions & 0x04) >> 2) > 0);

            public Int32 PictureSizeRestriction
            {
                get
                {
        ...
        {
            public Int32 Size = 0;                // Size of BOM
        ...
        static ID3v2()
        {
            ...
            // Mapping between standard ModifiedAtl fields and ID3v2.2 identifiers
        ...
        private Boolean readHeader(BufferedBinaryReader SourceFile, TagInfo Tag, Int64 offset)
        {
            var result = true;
        ...
        private Int32 getTagSize(TagInfo Tag, Boolean includeFooter = true)
        {
            // Get total tag size
            var result = StreamUtils.DecodeSynchSafeInt32(Tag.Size) + 10; // 10 being the size of the header
            ...
            Boolean inChapter = false)
        {
            ...
            var initialTagPos = source.Position;
            ...
            if (!Char.IsLetter(Frame.ID[0]) || !Char.IsUpper(Frame.ID[0]))
            {
                ...
                if (0 == Frame.ID[0] + Frame.ID[1] + Frame.ID[2])
                {
                    ...
                    var data = new Byte[PADDING_BUFFER_SIZE];
                    ...
                    while (!endReached)
                    {
                        ...
                        for (var i = 0; i < PADDING_BUFFER_SIZE; i++)
                        {
                            if (data[i] > 0)
                            {
            ...
            */
            ...
            if ("COM".Equals(Frame.ID.Substring(0, 3)))
            {
                ...
                var contentDescriptionBOM = new BOMProperties();
                ...
                dataSize = dataSize - (Int32)(source.Position - initialPos);
            }
            ...
            if (dataSize > 0)
            {
                ...
                {
                    String strData;
                    var shortFrameId = Frame.ID.Substring(0, 3);
                    ...
                    else if ("CHA".Equals(shortFrameId)) // Chapters
                    {
                        ...
                        var remainingData = dataSize - (source.Position - initPos);
                ...
                else // Picture frame
                {
                    ...
                    {
                        // Image format
                        var imageFormat = Utils.Latin1Encoding.GetString(source.ReadBytes(3)).ToUpper();
                    ...
                    else
                    {
                        ...
                        // Mime-type
                        var mimeType = StreamUtils.ReadNullTerminatedString(source, Utils.Latin1Encoding);
                    ...
                    var picType = DecodeID3v2PictureType(picCode);
                    ...
                    var description = StreamUtils.ReadNullTerminatedString(source, frameEncoding);

                    if (readTagParams.ReadPictures || readTagParams.PictureStreamHandler != null)
                    {
                        var picSize = dataSize - (Int32)(source.Position - position);

                        var picInfo = new PictureInfo(imgFormat, picType, getImplementedTagType(), picCode, picturePosition);
        ...
        private void readFrames(BufferedBinaryReader source, TagInfo tag, Int64 offset, ReadTagParams readTagParams)
        {
            Int64 streamPos;
            var streamLength = source.Length;
            var tagSize = getTagSize(tag, false);
            ...
            */
            if (comments != null && comments.Count > 0)
            {
                ...
                {
                    var commentDescription = comm.NativeFieldCode.Trim().Replace(Utils.UNICODE_INVISIBLE_EMPTY,"");
        ...
        /// <returns></returns>
        public Boolean Read(BinaryReader source, Int64 offset, ReadTagParams readTagParams)
        {
            ...
            ResetData();
            var result = readHeader(reader, tagHeader, offset);
            ...
            if (result && StreamUtils.StringEqualsArr(ID3V2_ID, tagHeader.ID))
            {
                ...
                if ((TAG_VERSION_2_2 <= tagVersion) && (tagVersion <= TAG_VERSION_2_4) && (getTagSize(tagHeader) > 0))
                {
                    ...
                    structureHelper.AddZone(offset, (Int32)(tagHeader.ActualEnd - offset));
                }
                else
                {
        ...
        internal Int32 writeInternal(TagData tag, BinaryWriter w, String zone)
        {
        ...
        protected override Int32 write(TagData tag, BinaryWriter w, String zone)
        {
            ...
            tagSize = (Int32)(finalTagPos - tagSizePos - 4);
        ...
        private Int32 writeExtHeaderAndFrames(TagData tag, BinaryWriter w)
        {
            var nbFrames = 0;
            Boolean doWritePicture;
            var tagEncoding = Settings.DefaultTextEncoding;
            ...
            foreach (var frameType in map.Keys)
            {
                if (map[frameType].Length > 0) // No frame with empty value
                {
                    ...
                    {
                        foreach (var s in frameMapping_v24.Keys)
                        {
            ...
            {
                var recordingTimestamp = new StringBuilder(recordingYear);
        ...
        private Int32 writeChapters(BinaryWriter writer, IList<ChapterInfo> chapters, Encoding tagEncoding)
        {
            Random randomGenerator = null;
            Int64 frameSizePos, finalFramePos, frameOffset;
            var frameHeaderSize = 6; // 4-byte size + 2-byte flags
            var result = 0;
            ...
            if ((Settings.ID3v2_alwaysWriteCTOCFrame && chapters.Count > 0) || AdditionalFields.ContainsKey("CTOC"))
            {
                ...
                w.Write((Int32)0); // Frame size placeholder to be rewritten in a few lines
                ...
                w.Write(StreamUtils.EncodeSynchSafeInt32((Int32)(finalFramePos - frameSizePos - frameOffset - frameHeaderSize)));
        ...
        private void writeTextFrame(BinaryWriter writer, String frameCode, String text, Encoding tagEncoding, String language = "", Boolean isInsideUnsynch = false)
        {
            String actualFrameCode; // Used for writing TXXX frames
            Int64 frameSizePos, finalFramePos, frameOffset;
            var frameHeaderSize = 6; // 4-byte size + 2-byte flags

            var isCommentCode = false;

            var writeValue = true;
            var writeTextEncoding = !noTextEncodingFields.Contains(frameCode);
            var writeNullTermination = true; // Required by specs; see §4, concerning $03 encoding
            ...
            var shortCode = frameCode.Substring(0, 3);
            ...
            else if (shortCode.Equals("POP")) // Rating frame specifics
            {
                ...
                // Play count
                w.Write((Int32)0); // TODO : handle this field dynamically. Warning : may be longer than 32 bits (see specs)
            ...
            writer.Write(StreamUtils.EncodeSynchSafeInt32((Int32)(finalFramePos - frameSizePos - frameOffset - frameHeaderSize)));
            writer.BaseStream.Seek(finalFramePos, SeekOrigin.Begin);
        }

        private void writePictureFrame(BinaryWriter writer, Byte[] pictureData, ImageFormat picFormat, String mimeType, Byte pictureTypeCode, String picDescription, Encoding tagEncoding, Boolean isInsideUnsynch = false)
        {
            ...
            Int64 frameSizePos2;
            Int64 finalFramePos;
            Int64 finalFramePosRaw;
            var dataSizeModifier = 0;
            ...
            frameSizePos = w.BaseStream.Position;
            w.Write((Int32)0); // Temp frame size placeholder; will be rewritten at the end of the routine
            ...
            */
            ...
            writer.Write(StreamUtils.EncodeSynchSafeInt32((Int32)(finalFramePos - frameSizePos - frameOffset - dataSizeModifier)));

            writer.BaseStream.Seek(frameSizePos2 + frameOffset, SeekOrigin.Begin);
            writer.Write(StreamUtils.EncodeSynchSafeInt32((Int32)(finalFramePosRaw - frameSizePos2 - dataSizeModifier)));
        ...
        private static String extractGenreFromID3v2Code(String iGenre)
        {
            ...
            var result = iGenre.Trim().Replace("\0","");
            var genreIndex = -1;
            ...
            for (var i=0;i < result.Length;i++)
            {
                ...
                {
                    if (Int32.TryParse(result.Substring(openParenthesisIndex + 1, i - openParenthesisIndex - 1), out genreIndex))
                    {
        ...
        private static Int32 readRatingInPopularityMeter(BufferedBinaryReader Source, Encoding encoding)
        {
        ...
        public static PictureInfo.PIC_TYPE DecodeID3v2PictureType(Int32 picCode)
        {
        ...
        private static Byte[] decodeUnsynchronizedStream(BufferedBinaryReader from, Int32 length)
        {
            ...
            var writeBuffer = new Byte[BUFFER_SIZE];
            ...
            if (length > 0)
            {
                remainingBytes = (Int32)Math.Min(length, from.Length - from.Position);
            }
            else
            {
                remainingBytes = (Int32)(from.Length - from.Position);
            }
